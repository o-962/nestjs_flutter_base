...
export class ApiResponse {
  ...
  serverError(options: {
    ...
    console.log('Called from:', new Error().stack); // This will show you where it's called from
    console.log('xxx');
  ...
  unAuthorized() {
    ...
    this.toast_body = 'UNAUTHORIZED';
    this.toast_body = 'UNAUTHORIZED';
    this.toast_type = 'alert';
    return this.buildResponse();
  }
  ...
  private checkToast() {
    if (this.toast_body || this.toast_header) {
      if (!this.toast_body || !this.toast_header) {
        throw new Error(
  ...
  static serviceResponse(
    ...
    if (
      options.status_code === HttpStatus.UNPROCESSABLE_ENTITY &&
      (!options.errors || Object.keys(options.errors).length === 0)
    ) {
